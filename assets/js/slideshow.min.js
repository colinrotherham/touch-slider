...
*/
	...
	J(V||{});b.init=function(){h=$(c.slideshow);f=h.find("."+c.classSlide);l=h.find("."+c.classStrip);k=f.length;if(h.length&&!(2>k)){x=l[0].style;b.slide=f.filter("."+c.classActive);b.index=f.index(b.slide);b.slide.length||(b.index=0,b.slide=f.eq(b.index));A=c.canLoop?Math.floor((k-1)/2):0;l.width(100*k+"%");f.width(100/k+"%");K&&Y();f.on("click touchend",p);h.mouseenter(w).mouseleave(n);E=h.find(c.next).on("click touchend",N);D=h.find(c.previous).on("click touchend",O);v=D.add(E);M=setTimeout(n,c.delay);
