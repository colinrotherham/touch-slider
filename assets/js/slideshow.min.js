...
*/
	...
	c.addClass(a.classTouch));q.width(100*d.length+"%");d.width(100/d.length+"%");for(var e=d.length,m;e--;)m=x(e,!0)+"%",d.eq(e).css({left:m,display:"block"}).attr("tabindex","-1");s(0);c.mouseenter(u).mouseleave(t);if(A){var f,n,l=function(a){var c=a.originalEvent,e=c.touches&&c.touches[0]||c;1<e.length||c.scale&&1!==c.scale||(f=(e.pageX||e.screenX)-h.x,n=(e.pageY||e.screenY)-h.y,"undefined"===typeof j&&(j=!!(j||Math.abs(f)<Math.abs(n))),j||(a.preventDefault(),u(),g=0<f,N()&&(f/=Math.abs(f)/b.width+
	1),s(0,void 0,f/b.width*(100/d.length))))},p=function(){j||((250>+new Date-h.time&&20<Math.abs(f)||Math.abs(f)>b.width/3)&&!N()?k(void 0,{isPrev:g}):s(a.time));c.off("touchmove MSPointerMove");c.off("touchend touchcancel MSPointerUp MSPointerOut")},h,j;c.on("touchstart MSPointerDown",function(b){var a=b.originalEvent,a=a.touches&&a.touches[0]||a;r.has(b.target).length||(v&&L(),h={x:a.pageX||a.screenX,y:a.pageY||a.screenY,time:+new Date},j=n=f=void 0,c.on("touchmove MSPointerMove",l),c.on("touchend touchcancel MSPointerUp MSPointerOut",
	p))});c.on("click",function(a){j||a.preventDefault()});$(window).resize(P)}else d.click(k),C=c.find(a.next).on("click",I),B=c.find(a.previous).on("click",J),O=B.add(C).css("display","block");if(a.classMarkers){r=$("<div />").addClass(a.classMarkers).on("click touchend","button",y);for(e=d.length;e--;)r.prepend($("<button>"+(e+1)+"</button>"));z=r.find("button");c.append(r)}K();y();P();H=setTimeout(t,a.delay)}};b.start=t;b.stop=u;b.next=I;b.prev=J;b.transition=s;b.getTransitionX=x};